...
<body id="body-pd">
   ...
   $req="SELECT * FROM reteur";

   $reponse=$conn->query($req);
   
    ?>
    <h1 style="border-bottom: 2px solid lightblue; width: 50%">liste des produits défectueux :</h1><br><br>
	 ...
	 echo $user['id_reteur']."</td><td>".$user['date_reteur']."</td><td>".$user['id_client']."</td><td>".$user['id_produit']."</td><td>".$user['quantité']."</td>";
	  ?>
	   <td><button class="btn btn-outline-danger" onclick="confirmer(<?php echo $user[4] ?>)">supprimer</button></td>
	  	<?php
     echo "<td><a  href='modifier/modifier_reteur.php?id=$user[4]'><button class=\"btn btn-outline-success\">modifier</button></a></td>";?>
  <td><a  href='../défectueux.php'><button class="btn btn-outline-primary">ajouter</button></a></td></tr>
<?php
   }?>
